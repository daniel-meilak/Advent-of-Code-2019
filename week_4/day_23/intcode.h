...
public:
   int pos_in_input;
   void run();
   void reset(std::list<long long> in);
...
void computer_t::inp(){
   ...
   //if (input.empty()){ input.push_back(-1LL); }
   if (input.empty()){
      running = false;
      input.push_back(-1LL);
      return;
   ...
   if      (mode[0]==0){ memory[a              ] = input.front(); }
   else if (mode[0]==2){ memory[a+rb           ] = input.front(); }
   else if (mode[0]==1){ memory[pos_in_memory+1] = input.front(); }
...
void computer_t::out(){
   ...
   if (output.size()==3){ running = false; }
   pos_in_memory += 2;
}
...
void computer_t::reset(std::list<long long> in){
   memory        = memory_save;
   rb            = 0;
   pos_in_input  = 0;
   pos_in_memory = 0;

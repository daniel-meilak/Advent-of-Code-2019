...
public:
   ...
   std::string status;
   long int output;
   computer_t(std::string filename, std::vector<long int> in, bool pause);
};

computer_t::computer_t(std::string filename, std::vector<long int> in, bool pause){
...
void computer_t::output_function(){
   ...
   pos_in_memory += 2;
   if (pause_on_output) running = false;
}

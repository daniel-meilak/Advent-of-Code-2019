...
public:
   ...
   std::string status;
   computer_t(const std::vector<long long> &opcode, std::vector<long long> in, int pause_type);
   computer_t(){};
};

computer_t::computer_t(const std::vector<long long> &opcode, std::vector<long long> in, int pause_type){
   ...
   // pause = 0   no pause
   // pause = 1   pause after output
   // pause = 2   pause before input
   pause         = pause_type;
   pause_buffer  = true;
}
